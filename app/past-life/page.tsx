...
export default function PastLifePage() {
    ...
    const handleStart = async () => {
        ...
        try {
            ...
            let wikiTitle = figure.wikiTitleEn;
            if (langCode === 'ko') wikiTitle = figure.wikiTitleKo;

            let res = await fetch(`https://${langCode}.wikipedia.org/api/rest_v1/page/summary/${wikiTitle}`);

            // Fallback to English if target language fails
            if (!res.ok && langCode !== 'en') {
                res = await fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${figure.wikiTitleEn}`);
            }
                            ...
                            >
                                <div className="glass-premium rounded-[2.5rem] border border-white/10 overflow-hidden relative shadow-2xl">
                                    ...
                                    <div className="p-8 md:p-12 space-y-10 relative">
                                        ...
                                        <div className="grid md:grid-cols-2 gap-10 items-start">
                                            ...
                                            <div className="space-y-6">
                                                ...
                                                <div className="pt-6 border-t border-white/5 grid grid-cols-2 gap-4">
                                                    <div>
                                                        <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest">{t('pastlife.label_category')}</p>
                                                        <p className="text-sm font-bold text-amber-200 uppercase">{t(`pastlife.cat_${result?.figure.category}`)}</p>
                                                    </div>
                                                    <div>
                                                        <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest">{t('pastlife.label_resonance')}</p>
                                        ...
                                        <div className="flex justify-between items-center pt-8 border-t border-white/5 text-[9px] text-white/20 uppercase tracking-[0.2em] font-bold">
                                            <span>{t('pastlife.verified')}</span>
                                            <span className="flex items-center gap-1"><Globe className="w-3 h-3" /> {t('pastlife.encrypted')}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col gap-3">
                                    ...
                                    >
                                        ...
                                        {copied ? t('pastlife.copied') : t('pastlife.copy_link')}
                                    </button>
                                    <button
